...
- Hands-on AI development: LLM governance layers (LiteLLM, NeMo Guardrails, Ollama), prompt engineering as code (versioned prompts, interface contracts, scoring rubrics), AI agent architecture (multi-source scraping, heuristic gating, cost-optimized LLM pipelines) and composable LLM workflow design. Built with Claude Code as development accelerator, maintaining architecture ownership through ADRs, patterns and schemas.
