...
Verifies:
(a) Job with domain='game' and user weight game:-25 → negative domain contribution.
(b) Job with domain='other' and game keywords → inferred to 'game' → negative weight applied.
...
class TestNegativeDomainInHeuristic:
    ...
    def test_game_domain_negative_weight_applied(self):
        ...
        # domain explicitly 'game', no skills to add noise
        parsed = {
            "domain": "game",
            "seniority": "unknown",
    ...
    def test_game_job_ranks_lower_than_data_job(self):
        """Game job (negative domain) must score lower than data job (positive domain)."""
        profile = _make_profile()
        game_parsed = {
            "domain": "game",
            "seniority": "senior",
        ...
        assert game_score < data_score, (
            f"Game job ({game_score}) should score lower than data job ({data_score})"
        )


class TestDomainInferenceWithNegativeWeight:
    """domain='other' + game keywords → inferred game → negative weight flows through."""
    ...
    def test_other_domain_with_game_keywords_inferred(self):
        ...
        # Verify _infer_domain picks up 'game'
    ...
    def test_other_with_game_keywords_applies_negative_weight(self):
        ...
        # With game:-25, result should be clamped to 0 (not receiving 0 from unknown domain)
        ...
        # game:-25 should not produce a higher score than having no weight for game
...
class TestScoreClampedToZero:
    ...
    def test_heavy_negatives_clamped_to_zero(self):
        profile = _make_profile(domains={"game": -100})
        parsed = {
            "domain": "game",
            "seniority": "unknown",
