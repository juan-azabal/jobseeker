...
def _make_docx_with_em_dash(tmp_path) -> str:
    ...
    for header in ["Work Experience", "Education and Certifications",
                   "Languages", "Selected Impact", "Core Skills"]:
...
def _make_docx_all_bold(tmp_path) -> str:
    ...
    for header in ["Summary", "Selected Impact", "Core Skills",
...
def test_clean_docx_has_no_warnings(tmp_path):
    ...
    assert result["warnings"] == [], (
        f"Expected no warnings for clean Phase 6 docx. Got: {result['warnings']}"
    )
