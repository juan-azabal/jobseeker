...
test('shows jobs page when authenticated', async () => {
  global.fetch = vi.fn()
    .mockResolvedValueOnce({
      ok: true,
      json: async () => ({ id: 1, email: 'u@e.com', name: 'User', avatar_url: null, profile_id: 'user', onboarded: true }),
    } as Response)
    .mockResolvedValue({
      ok: true,
...
test('shows user name in header when authenticated', async () => {
  global.fetch = vi.fn()
    .mockResolvedValueOnce({
      ok: true,
      json: async () => ({ id: 1, email: 'u@e.com', name: 'Alice', avatar_url: null, profile_id: 'alice', onboarded: true }),
    } as Response)
    .mockResolvedValue({
      ok: true,
      json: async () => ({ jobs: [], total: 0, filters: { tier: [], period: 'all' } }),
